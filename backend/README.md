...
├── db/                  # SQLite connection management, schema introspection, seeder
├── rag/                 # FAISS vector store and few-shot document corpus
...
- **Agent pipeline** — Classifier, Guardrails, and RAG run in parallel (fan-out), then Schema Agent → SQL Agent ↔ Validation Agent → SQLite execution → Insight Agent
- **Chat history** — persisted in MongoDB (sessions, messages, feedback)
- **FMCG data** — queried via SQLite (read-only for LLM-generated SQL)
...
|-------|--------|----------------|
| FMCG data | SQLite | Products, orders, sales, inventory — the NL → SQL query target |
| Chat history | MongoDB | Sessions, messages, user feedback (thumbs up/down) |
| RAG examples | FAISS | NL → SQL few-shot pairs, curated via Operations Center |
...
cp .env.example .env
# configure LLM endpoint and MongoDB URI in .env
...
- [ ] `requirements.txt` needs LangGraph, motor (async MongoDB), faiss-cpu, and other dependencies
