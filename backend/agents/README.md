...
This folder contains every agent in the pipeline. Each agent is a focused module with a single responsibility. They are composed into a LangGraph workflow defined in `backend/graph/pipeline.py`, which handles orchestration, parallel fan-out, and the validation retry loop.
...
| `rag_agent.py` | Retrieves few-shot SQL examples from the FAISS vector store |
...
- **LangGraph orchestration** — graph-based workflow with native support for parallel branches, conditional edges, and checkpointing. Defined in `backend/graph/pipeline.py`.
...
- **Model-agnostic** — all agents call the LLM endpoint configured in `backend/config/settings.py`. Works with Ollama, LM Studio, or any OpenAI-compatible API.
